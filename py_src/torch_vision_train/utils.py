...
class SmoothedValue:
    ...
    def synchronize_between_processes(self):
        """Warning: this does not synchronize the deque itself."""
...
def store_model_weights(model, checkpoint_path, checkpoint_key="model", strict=True):
    ...
    output_path = os.path.join(output_dir, "weights-" + str(digest[:8]) + ".pth")
...
def set_weight_decay(
    ...
    if custom_keys_weight_decay is not None:
        for key, wd in custom_keys_weight_decay:
            params[key] = []
            params_weight_decay[key] = wd
