...
def evaluate_model(current_ml_setup, *, device: Device, batch_size: int, num_workers: int, num_samples: Optional[int]) -> ValResult:
...
def _write_report(
    ...
    *,
    ...
    with open(output_path, "w", encoding="utf-8") as outfile:
        ...
        outfile.write(f"val loss={val_result.avg_loss}, val acc={_format_accuracy(val_result)}\n")
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Measure model validation accuracy and loss.")
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    print(f"val loss={val_result.avg_loss}, val acc={_format_accuracy(val_result)}")
