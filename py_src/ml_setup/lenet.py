...
def lenet5_mnist() -> MLSetup:
    ...
    return make_setup(model, ModelType.lenet5, dataset_mnist(), 64, has_normalization=False)


def lenet4_mnist() -> MLSetup:
    model = LeNet4()
    model.apply(weights_init_xavier)
    return make_setup(model, ModelType.lenet4, dataset_mnist(), 64, has_normalization=False)


def lenet5_large_fc_mnist() -> MLSetup:
    model = LeNet5LargeFc()
    model.apply(weights_init_xavier)
    return make_setup(model, ModelType.lenet5_large_fc, dataset_mnist(), 64, has_normalization=False)
